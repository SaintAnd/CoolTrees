...
onready var polygon = get_parent()	# получаем область, за которую светлячок не может залететь
...
func fly():	# эта функия вызывается при помощи сигнала timeout() у таймера который висит на светляке
	...
	if Geometry.is_point_in_polygon(pos, polygon.polygon):	# проверяем находится ли эта точка в области перемещения
